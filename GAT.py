...
class GAT(torch.nn.Module):
    ...
    """
    def __init__(self, dim_in:int, dim_out:int, dim_h1:int=32, dim_h2:int=0, heads:int=4, dropout:float=0.4, lr:float=0.001, wd:float=5e-4, logger=None):   
        """
        Constructor
        """
        super().__init__()
        ...
        ### Define the model
        # Layer 1: Conv
        self.conv1 = GATConv(dim_in, dim_h1 // heads, heads=heads, dropout=dropout)
        # Layer 2: Conv
        if dim_h2 > 0:
            self.conv2 = GATConv(dim_h1, dim_h2 // heads, heads=heads, dropout=dropout)
        else:
            self.conv2 = None
        # Layer 3: Linear
        if dim_h2 > 0:
    ...
    def forward(self, data):
        ...
        h = F.elu(h)  # GAT uses (normally) ELU activation
    ...
    def test_binary(self, test_graphs, y_test):
        """
        Test the model
        """
    ...
    def _print_info(self):
        ...
        self._logger.info("Layer 1: GATConv({}, {})".format(self.conv1.in_channels, self.conv1.out_channels))
        if self.conv2 is not None:
            self._logger.info("Layer 2: GATConv({}, {})".format(self.conv2.in_channels, self.conv2.out_channels))
